...
<!-- Card -->
<div
	class="w-92 h-auto bg-gray-800 rounded-lg shadow-md overflow-hidden mb-4 flex flex-col justify-between transform transition duration-200 hover:translate-y-1 hover:opacity-90"
>
	...
	<!-- Download button -->
	<a
		href={loaded ? `./midi/${id}.mid` : "#"}
		class="w-full"
		target="_blank"
		download={loaded ? `${title}.mid` : ""}
	>
		<button
			...
			disabled={!loaded}
			style="opacity: {loaded ? 1 : 0.5}; cursor: {loaded
				? 'pointer'
				: 'not-allowed'};"
		>
